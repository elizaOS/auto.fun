...
import { createNewTokenData } from "./utils";
...
export async function resumeOnStart(env: Env, connection: Connection) {
  ...
  const program = new Program<RaydiumVault>(raydium_vault_IDL as any, provider);
  ...
  try {
    ...
    if (mintLog && swapLog && reservesLog && feeLog && swapeventLog) {
      try {
        ...
        const tokenPriceUSD =
          currentPrice > 0
            ? (currentPrice / 10 ** TOKEN_DECIMALS) *
              solPrice *
              10 ** TOKEN_DECIMALS
            : 0;
    ...
    if (completeEventLog && mintLog) {
      try {
        ...
        const token = await getToken(env, mintAddress);
        if (!token) {
          throw new Error(`Token not found in database: ${mintAddress}`);
        }
...
export async function addOneToken(env: any) {
  const { signature, rawTokenAddress, rawCreatorAddress } = {
    signature:
      "5QVEi1gWwjVThsjkru5jmzkXexsXB2qrqXk3yfZjmFED3RabTnZpKeyAhfaN4ttgBgypN45QzUt6W39FpRfUkFG8",
...
const list = [
  {
    ...
    signature:
      "333kTadpSPENQQnEeaAWfqZYaqWBkL4AV9Z8WWdM25ZEGDMVdGMuU8mdQtcG8MHWBN2YNCocd5biB2pd3NBxAKrL",
  },
];
export async function migrateTokensFromList(env: Env, connection: Connection) {
