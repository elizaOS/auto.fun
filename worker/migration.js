...
import { CREATE_CPMM_POOL_FEE_ACC, CREATE_CPMM_POOL_PROGRAM, DEV_LOCK_CPMM_AUTH, DEV_LOCK_CPMM_PROGRAM, DEVNET_PROGRAM_ID, getCpmmPdaAmmConfigId, } from "@raydium-io/raydium-sdk-v2";
 ...
 */
export function createMigrationService(connection, programId, wallet, env) {
    ...
    const socketAdapter = {
        to: (room) => ({
            emit: (event, data) => wsClient.emit(room, event, data)
        }),
        // Add any other required socket.io methods here
    };
    return new MigrationService(connection, mockProgram, wallet, env, socketAdapter);
}
...
function storeMigrationMetadata(token, _stateJson, db) {
...
export class MigrationService {
    ...
    io;
    env;
    constructor(connection, program, wallet, env, io) {
    ...
    async migrateToken(token) {
        try {
            ...
            if (!token.migration.withdraw ||
                token.migration.withdraw.status !== "success") {
                logger.log(`[Migrate] Starting withdrawal for token ${token.mint}`);
                const withdrawResult = await retryOperation(() => this.performWithdraw(token), 3, 2000);
                ...
                logger.log(`[Migrate] Withdrawal successful for token ${token.mint} txId: ${withdrawResult.txId}`);
            }
            else {
                logger.log(`[Migrate] Withdrawal already processed for token ${token.mint}`);
            }
            // Step 2: Create pool
            if (!token.migration.createPool ||
                token.migration.createPool.status !== "success") {
                logger.log(`[Migrate] Starting pool creation for token ${token.mint}`);
                const poolResult = await retryOperation(() => this.performCreatePool(token), 3, 2000);
                ...
                await storeMigrationMetadata(token, JSON.stringify(token.migration), db);
                logger.log(`[Migrate] Pool creation successful for token ${token.mint} txId: ${poolResult.txId}`);
            }
            else {
                logger.log(`[Migrate] Pool creation already processed for token ${token.mint}`);
            }
            // Step 3: Lock LP tokens
            if (!token.migration.lockLP ||
                token.migration.lockLP.status !== "success") {
                logger.log(`[Migrate] Starting LP token locking for token ${token.mint}`);
                const lockResult = await retryOperation(() => this.performLockLP(token), 3, 2000);
                ...
                await storeMigrationMetadata(token, JSON.stringify(token.migration), db);
                logger.log(`[Migrate] LP token locking successful for token ${token.mint} txId: ${lockResult.txId}`);
            }
            else {
                logger.log(`[Migrate] LP token locking already processed for token ${token.mint}`);
            }
            // Step 4: Finalize migration
            if (!token.migration.finalize ||
                token.migration.finalize.status !== "success") {
                logger.log(`[Migrate] Finalizing migration for token ${token.mint}`);
                const finalizeResult = await retryOperation(() => this.performFinalizeMigration(token), 3, 2000);
                ...
                logger.log(`[Migrate] Migration finalization already processed for token ${token.mint}`);
            }
            if (this.io) {
    ...
    parseWithdrawLogs(withdrawLogs) {
        let withdrawnSol = 0;
    ...
    async performLockLP(token) {
        ...
        const lpAccount = raydium.account.tokenAccounts.find((a) => a.mint.toBase58() === lpMintStr);
        if (!lpAccount) {
    ...
    async fetchPoolInfoWithRetry(raydium, poolId) {
    ...
    async performFinalizeMigration(_token) {
