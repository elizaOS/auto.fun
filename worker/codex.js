 ...
 */
export async function fetchCodexTokenEvents(tokenAddress, startTimestamp, endTimestamp, networkId = 1399811149) {
 ...
 */
export async function fetchCodexTokenPrice(tokenAddress, networkId = 1399811149) {
    const apiUrl = 'https://graph.codex.io/graphql';
    const query = `query {
    getTokens(
      query: {
 ...
 */
export function convertCodexEventsToPriceFeed(events) {
 ...
 */
export async function fetchCodexBars(tokenAddress, startTimestamp, endTimestamp, resolution = '1', networkId = 1399811149, quoteToken = 'token1') {
    const apiUrl = 'https://graph.codex.io/graphql';
    ...
    if (totalPoints <= MAX_DATA_POINTS) {
        ...
        for (let chunkStart = startTimestamp; chunkStart < endTimestamp; chunkStart += chunkSize) {
            ...
            const chunkPromise = fetchCodexBarsChunk(apiUrl, symbol, chunkStart, chunkEnd, resolution, quoteToken, timeInterval).catch(error => {
 ...
 * @private
 */
async function fetchCodexBarsChunk(apiUrl, symbol, startTimestamp, endTimestamp, resolution, quoteToken, timeInterval) {
    const query = `query {
    getBars(
      symbol: "${symbol}"
      from: ${Math.floor(startTimestamp)}
