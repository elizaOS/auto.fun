...
export class WebSocketDO {
    ...
    try {
      ...
      if (token.includes(".")) {
        ...
        try {
          ...
          const response = await fetch(`${this.env.VITE_API_URL}/api/auth-status`, {
            headers: {
  ...
  private async handleSearchTokens(
    clientId: string,
    data: any
  ): Promise<void> {
    try {
      ...
      logger.log(`WebSocket search request from client ${clientId}: "${searchQuery}"`);
      ...
      try {
        ...
        if (response.ok) {
          ...
          logger.log(`Search results for "${searchQuery}": found ${searchData.tokens?.length || 0} tokens`);
  ...
  private async handleTokenMetrics(
    clientId: string,
    data: any
  ): Promise<void> {
    try {
  ...
  private async handleTokenData(
    ...
    try {
      ...
      logger.log(`WebSocket request for token data: ${mint} (bypass cache: ${bypassCache})`);
  ...
  private async handleWalletBalance(
    ...
    try {
      ...
      try {
        ...
        const url = new URL(`${this.env.VITE_API_URL}/api/wallet/${address}/balance`);
        
        const response = await fetch(url.toString());
        
        if (response.ok) {
          const balanceData = await response.json();
          logger.log(`Balance for ${address} fetched successfully: ${balanceData.balance}`);
          return this.sendToClient(clientId, "balanceUpdate", {
            address,
  ...
  private async handleTokenBalance(
    ...
    try {
      ...
      logger.log(`WebSocket request for token balance: ${mint} for wallet ${address}`);
      
      try {
        // Call the API to get token balance
        const url = new URL(`${this.env.VITE_API_URL}/api/wallet/${address}/token/${mint}`);
        
        const response = await fetch(url.toString());
        
        if (response.ok) {
          const balanceData = await response.json();
          logger.log(`Token balance for ${mint} (owner ${address}) fetched successfully: ${balanceData.balance}`);
          return this.sendToClient(clientId, "tokenBalanceUpdate", {
            address,
            mint,
            balance: balanceData.balance,
