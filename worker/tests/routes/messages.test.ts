...
describe("Messages API Endpoints", () => {
  ...
  beforeAll(async () => {
    ...
    if (nonceResponse.response.status !== 200) {
      throw new Error(
        `Failed to generate nonce: ${nonceResponse.response.status}`,
      );
    }
    ...
    if (authResponse.response.status !== 200 || !authResponse.data?.token) {
      throw new Error(
        `Failed to authenticate: ${authResponse.response.status}`,
      );
    }

    authToken = authResponse.data.token;
    console.log(
      "Successfully authenticated with signature, token:",
  ...
  it("validates API endpoints", async () => {
    if (!ctx.context) throw new Error("Test context not initialized");
    if (!authToken)
      throw new Error("No auth token available - authentication failed");
    ...
    expect(createData).toHaveProperty(
      "message",
      "Test message with proper authentication",
    );
  });
  ...
  it("checks message listing and thread access", async () => {
    ...
    const { response: messagesResponse, data: messagesData } =
      await fetchWithAuth(apiUrl(baseUrl, `/messages/${testMint}`), "GET");
