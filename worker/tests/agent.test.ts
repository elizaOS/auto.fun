...
describe('Agent API Endpoints', () => {
  ...
  it('should upload an image to Cloudflare', async () => {
    ...
    const smallImage = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=';
    ...
    const { response } = await fetchWithAuth<{ success: boolean, imageUrl: string, metadataUrl: string }>(
      apiUrl(baseUrl, '/upload-cloudflare'),
      'POST',
      uploadRequest
    );
