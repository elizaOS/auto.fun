...
import { apiUrl, fetchWithAuth, sleep } from './helpers/test-utils';
...
describe('Media Generation API Endpoints', () => {
    ...
    beforeAll(async () => {
        ...
        if (testState.tokenPubkey) {
            ...
            const response = await fetchWithAuth(apiUrl(baseUrl, '/new_token'), 'POST', {
        ...
        const nonceResponse = await fetchWithAuth(apiUrl(baseUrl, '/generate-nonce'), 'POST', { publicKey });
        if (nonceResponse.response.status === 200 && nonceResponse.data && nonceResponse.data.nonce) {
            ...
            // Authenticate
            const authResponse = await fetchWithAuth(apiUrl(baseUrl, '/authenticate'), 'POST', { publicKey, signature });
    ...
    it('should generate an image', async () => {
        ...
        const { response, data } = await fetchWithAuth(apiUrl(baseUrl, `/${tokenMint}/generate`), 'POST', generationRequest, undefined, headers);
        if (response.status === 200) {
            ...
            expect(data.limit).toBe(RATE_LIMITS[MediaType.IMAGE].MAX_GENERATIONS_PER_DAY);
        }
        else {
    ...
    it('should handle rate limits for generations', async () => {
        ...
        for (let i = 0; i < 5; i++) {
            const result = await fetchWithAuth(apiUrl(baseUrl, `/${tokenMint}/generate`), 'POST', {
    ...
    it('should fetch generation history', async () => {
        ...
        const { baseUrl } = ctx.context;
        const headers = authToken ? { Authorization: `Bearer ${authToken}` } : undefined;
        const { response, data } = await fetchWithAuth(apiUrl(baseUrl, `/${tokenMint}/history`), 'GET', undefined, undefined, headers);
        if (response.status === 200) {
    ...
    it('should handle video generation request', async () => {
        ...
        const { response, data } = await fetchWithAuth(apiUrl(baseUrl, `/${tokenMint}/generate`), 'POST', videoRequest, undefined, headers);
        if (response.status === 200) {
            expect(data).toHaveProperty('success');
            expect(data).toHaveProperty('mediaUrl');
            expect(data.success).toBe(true);
            ...
            expect(data).toHaveProperty('error');
            expect(data).toHaveProperty('limit');
            expect(data.limit).toBe(RATE_LIMITS[MediaType.VIDEO].MAX_GENERATIONS_PER_DAY);
        }
        else {
    ...
    it('should handle audio generation request', async () => {
        ...
        const { response, data } = await fetchWithAuth(apiUrl(baseUrl, `/${tokenMint}/generate`), 'POST', audioRequest, undefined, headers);
        if (response.status === 200) {
            expect(data).toHaveProperty('success');
            expect(data).toHaveProperty('mediaUrl');
            expect(data.success).toBe(true);
            ...
            expect(data).toHaveProperty('error');
            expect(data).toHaveProperty('limit');
            expect(data.limit).toBe(RATE_LIMITS[MediaType.AUDIO].MAX_GENERATIONS_PER_DAY);
        }
        else {
