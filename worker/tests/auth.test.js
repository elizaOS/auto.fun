...
describe('Authentication API Endpoints', () => {
    ...
    it('should generate a nonce for authentication', async () => {
        ...
        const { response, data } = await fetchWithAuth(apiUrl(baseUrl, '/generate-nonce'), 'POST', { publicKey });
    ...
    it('should authenticate with a valid signature', async () => {
        ...
        const { response, data } = await fetchWithAuth(apiUrl(baseUrl, '/authenticate'), 'POST', { publicKey, signature, nonce });
        expect(response.status).toBe(200);
    ...
    it('should reject authentication with an invalid signature', async () => {
        ...
        const { response } = await fetchWithAuth(apiUrl(baseUrl, '/authenticate'), 'POST', { publicKey, signature: invalidSignature, nonce });
    ...
    it('should show unauthenticated status without a valid token', async () => {
        if (!ctx.context)
            throw new Error('Test context not initialized');
        const { baseUrl } = ctx.context;
        const { response, data } = await fetchWithAuth(apiUrl(baseUrl, '/auth-status'), 'GET');
        expect(response.status).toBe(200);
    ...
    it('should handle auth status and logout endpoints', async () => {
        ...
        try {
            ...
            if (!nonce) {
                const nonceResponse = await fetchWithAuth(apiUrl(baseUrl, '/generate-nonce'), 'POST', { publicKey: userKeypair.publicKey.toBase58() });
            ...
            const signatureBytes = nacl.sign.detached(messageBytes, userKeypair.secretKey);
            const signature = bs58.encode(signatureBytes);
            const authResponse = await fetchWithAuth(apiUrl(baseUrl, '/authenticate'), 'POST', {
        ...
        const statusResponse = await fetchWithAuth(apiUrl(baseUrl, '/auth-status'), 'GET', undefined, undefined, headers);
        // Try logout
        const logoutResponse = await fetchWithAuth(apiUrl(baseUrl, '/logout'), 'POST', undefined, undefined, headers);
        // We're just confirming endpoints exist and respond
        expect(statusResponse.response.status).toBe(200);
        expect(logoutResponse.response.status === 200 || logoutResponse.response.status === 401).toBeTruthy();
    });
});
