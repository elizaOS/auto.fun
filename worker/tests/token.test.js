...
describe('Token API Endpoints', () => {
    ...
    it('should return API info', async () => {
        ...
        const { response } = await fetchWithAuth(apiUrl(baseUrl, '/info'), 'GET');
    ...
    it('should create a new token', async () => {
        ...
        try {
            ...
            const { response } = await fetchWithAuth(apiUrl(baseUrl, '/token/create'), 'POST', tokenRequest, apiKey);
    ...
    it('should fetch token information', async () => {
        ...
        if (!testState.tokenPubkey)
            throw new Error('Token pubkey not available');
        const { baseUrl } = ctx.context;
        const { response } = await fetchWithAuth(apiUrl(baseUrl, `/token/${testState.tokenPubkey}`), 'GET');
    ...
    it('should execute a swap operation', async () => {
        ...
        const { response, data } = await fetchWithAuth(apiUrl(baseUrl, '/swap'), 'POST', swapRequest, apiKey);
        expect(response.status).toBe(200);
    ...
    it('should fetch token price', async () => {
        ...
        if (!testState.tokenPubkey)
            throw new Error('Token pubkey not available');
        const { baseUrl } = ctx.context;
        const { response } = await fetchWithAuth(apiUrl(baseUrl, `/token/${testState.tokenPubkey}/price`), 'GET');
        // We're just testing the endpoint responds
        expect(response.status).toBe(200);
    });
    it('should fetch a list of tokens', async () => {
        if (!ctx.context)
            throw new Error('Test context not initialized');
        const { baseUrl } = ctx.context;
        const { response } = await fetchWithAuth(apiUrl(baseUrl, '/tokens'), 'GET');
        // We're just testing the endpoint responds
        expect(response.status).toBe(200);
    });
    it('should filter tokens by status', async () => {
        ...
        const { response } = await fetchWithAuth(apiUrl(baseUrl, '/tokens?status=pending'), 'GET');
        // We're just testing the endpoint responds
        expect(response.status).toBe(200);
    });
    it('should search tokens by keyword', async () => {
        ...
        const { response } = await fetchWithAuth(apiUrl(baseUrl, `/tokens?search=${TEST_NAME}`), 'GET');
        // We're just testing the endpoint responds
        expect(response.status).toBe(200);
    });
    it('should fetch token holders', async () => {
        ...
        if (!testState.tokenPubkey)
            throw new Error('Token pubkey not available');
        const { baseUrl } = ctx.context;
        const { response, data } = await fetchWithAuth(apiUrl(baseUrl, `/tokens/${testState.tokenPubkey}/holders`), 'GET');
        expect(response.status).toBe(200);
    ...
    it('should fetch token swaps history', async () => {
        ...
        if (!testState.tokenPubkey)
            throw new Error('Token pubkey not available');
        const { baseUrl } = ctx.context;
        const { response, data } = await fetchWithAuth(apiUrl(baseUrl, `/swaps/${testState.tokenPubkey}`), 'GET');
        expect(response.status).toBe(200);
    ...
    it('should fetch token messages', async () => {
        ...
        if (!testState.tokenPubkey)
            throw new Error('Token pubkey not available');
        const { baseUrl } = ctx.context;
        const { response } = await fetchWithAuth(apiUrl(baseUrl, `/messages/${testState.tokenPubkey}`), 'GET');
        // We're just testing the endpoint responds
        expect(response.status).toBe(200);
    });
    it('should create a new message for a token', async () => {
        ...
        const { response } = await fetchWithAuth(apiUrl(baseUrl, `/messages/${testState.tokenPubkey}`), 'POST', messageRequest, undefined, { Authorization: 'Bearer test_token' } // Mock auth token
        );
        ...
        if (response.status === 401) {
            ...
            expect(data.message).toBe(messageRequest.message);
        }
    });
    it('should fetch chart data for a token', async () => {
        ...
        const { response, data } = await fetchWithAuth(apiUrl(baseUrl, `/chart/${pairIndex}/${start}/${end}/${range}/${testState.tokenPubkey}`), 'GET');
        expect(response.status).toBe(200);
