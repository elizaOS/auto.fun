...
import { ComputeBudgetProgram, Connection, Keypair, PublicKey, SystemProgram, SYSVAR_RENT_PUBKEY, TransactionInstruction } from "@solana/web3.js";
...
import { getDB, tokenHolders, tokens } from "./db";
 ...
 */
export const fetchMetadataWithBackoff = async (umi, tokenAddress, env) => {
...
export async function getTxIdAndCreatorFromTokenAddress(tokenAddress, env) {
    console.log(`tokenAddress: ${tokenAddress}`);
    // Get a Solana config with the right environment
    const solanaConfig = initSolanaConfig(env);
    const transactionHistory = await solanaConfig.connection.getSignaturesForAddress(new PublicKey(tokenAddress));
 ...
 */
export async function createNewTokenData(txId, tokenAddress, creatorAddress, env) {
    try {
        // Get a Solana config with the right environment
        const solanaConfig = initSolanaConfig(env);
        ...
        const [bondingCurvePda] = PublicKey.findProgramAddressSync([Buffer.from(SEED_BONDING_CURVE), new PublicKey(tokenAddress).toBytes()], solanaConfig.programId);
        ...
        const tokenData = {
            ...
            liquidity: ((Number(bondingCurveAccount.reserveLamport) / 1e9 * solPrice) +
 ...
 */
export async function bulkUpdatePartialTokens(tokens, env) {
    ...
    const updatedTokensPromises = tokens.map(token => calculateTokenMarketData(token, solPrice));
    // Wait for all updates to complete
    return Promise.all(updatedTokensPromises);
}
export const createConfigTx = async (admin, newConfig, connection, program) => {
...
export const launchTokenTx = async (decimal, supply, reserve, name, symbol, uri, user, connection, program, env) => {
    ...
    const tx = await program.methods
        .launch(
    //  launch config
    decimal, new BN(supply), new BN(reserve), 
    //  metadata
    name, symbol, uri)
        .accounts({
        creator: user,
...
export const swapTx = async (user, token, amount, style, slippageBps = 100, connection, program) => {
    const [configPda, _] = PublicKey.findProgramAddressSync([Buffer.from(SEED_CONFIG)], program.programId);
    const configAccount = await program.account.config.fetch(configPda);
    const [bondingCurvePda] = PublicKey.findProgramAddressSync([Buffer.from(SEED_BONDING_CURVE), token.toBytes()], program.programId);
    ...
    let estimatedOutput;
    if (style === 0) { // Buy
        estimatedOutput = calculateAmountOutBuy(curve.reserveToken.toNumber(), adjustedAmount, curve.reserveLamport.toNumber(), feePercent);
    }
    else { // Sell
        estimatedOutput = calculateAmountOutSell(curve.reserveLamport.toNumber(), adjustedAmount, feePercent, curve.reserveToken.toNumber());
    }
...
export const withdrawTx = async (user, token, connection, program) => {
    const tx = await program.methods
        .withdraw()
        .accounts({
        admin: user,
...
export const execTx = async (transaction, connection, payer, commitment = 'confirmed') => {
    try {
...
export async function execWithdrawTx(tx, connection, wallet, maxRetries = 1) {
    let lastError = null;
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            if (confirmation.value.err === 'ProgramFailedToComplete' ||
                ...
                if (txInfo?.meta?.logMessages?.some(log => log.includes(`Program success`))) {
...
export const getATokenAccountsNeedCreate = async (connection, walletAddress, owner, nfts) => {
    const instructions = [];
...
export async function updateHoldersCache(env, mint) {
    try {
        const db = getDB(env);
        const connection = new Connection(getRpcUrl(env));
        // Get token holders from Solana
        const accounts = await connection.getParsedProgramAccounts(new PublicKey('TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA'), // Token program
        {
            filters: [
                {
