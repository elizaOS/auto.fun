...
export class LRUCacheService {
  ...
  async lpushTrim(
    ...
    logger.info(`LPUSH+LTRIM on ${this.getKey(key)} limit ${maxLength}`);
    const list = await this.getList(key);
    list.unshift(value);
...
export function createLRUCache(env: Env, options: CacheOptions = {}) {
  if (!lruCacheInstance) {
    logger.info("Creating LRU cache service");
    lruCacheInstance = new LRUCacheService(env, options);
    logger.info("LRU cache service created");
  }
