...
export async function checkRateLimits(env, mint, type) {
    const db = getDB(env);
    ...
    const recentGenerationsCount = await db.select({ count: sql `count(*)` })
        ...
        .where(and(eq(mediaGenerations.mint, mint), eq(mediaGenerations.type, type), gte(mediaGenerations.timestamp, cutoffTime)));
...
export async function generateMedia(falApiKey, data) {
    ...
    try {
        ...
        if (!allowed) {
            return c.json({
                ...
                message: `You can generate up to ${RATE_LIMITS[validatedData.type].MAX_GENERATIONS_PER_DAY} ${validatedData.type}s per day`
            }, 429);
        }
        ...
        const typedResult = result; // Type casting for safety
...
app.get('/:mint/history', async (c) => {
    try {
        ...
        const cutoffTime = new Date(Date.now() - RATE_LIMITS[type || MediaType.IMAGE].COOLDOWN_PERIOD_MS).toISOString();
        ...
        return c.json({
            ...
            resetTime: new Date(Date.now() + RATE_LIMITS[type || MediaType.IMAGE].COOLDOWN_PERIOD_MS).toISOString()
        });
    }
    catch (error) {
