...
class RedisCacheService {
   ...
   async get(key: string): Promise<string | null> {
      return this.redisPool.useClient((client) => client.get(key));
   }

   async set(key: string, value: string, ttlInSeconds?: number): Promise<"OK" | null> {
      return this.redisPool.useClient((client) => {
         return ttlInSeconds
            ? client.set(key, value, "EX", ttlInSeconds)
            : client.set(key, value);
      });
   }

   async del(key: string): Promise<number> {
      return this.redisPool.useClient((client) => client.del(key));
   }
   ...
   async ttl(key: string): Promise<number> {
      return this.redisPool.useClient((client) => client.ttl(key));
   }
}
...
export default {
   async fetch(request: Request, env: any, ctx: ExecutionContext) {
      ...
      if (pathname.startsWith("/set/")) {
         ...
         if (!key || !value) return new Response("Key or value missing", { status: 400 });
