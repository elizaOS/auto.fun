...
async function updateAndBroadcastSolPrice(env: Env) {
  try {
    ...
    if (
      DEV_SOL_PRICE_CACHE.price &&
      DEV_SOL_PRICE_CACHE.connectedClients.size > 0
    ) {
      ...
      logger.log(
        `Dev WebSocket: Broadcasted SOL price $${DEV_SOL_PRICE_CACHE.price} to ${deliveredCount} clients`,
      );
    }
  } catch (error) {
      ...
      if (env.WEBSOCKET_DO) {
        ...
        try {
          ...
          server.addEventListener("message", (event) => {
            try {
              ...
              if (!server.__clientId) {
                ...
                logger.log(
                  `Assigned WebSocket client ID: ${server.__clientId}`,
                );
              }
              ...
              try {
                ...
                if (message.clientId) {
                  ...
                  logger.log(
                    `Updated WebSocket client ID to: ${server.__clientId}`,
                  );
                }
                ...
                if (message.event === "subscribeGlobal") {
                  ...
                  const handleSolPriceRequest = async () => {
                    try {
                      ...
                      if (
                        DEV_SOL_PRICE_CACHE.price &&
                        now - DEV_SOL_PRICE_CACHE.lastUpdate <
                          DEV_SOL_PRICE_CACHE.cacheValidity
                      ) {
                        price = DEV_SOL_PRICE_CACHE.price;
                  ...
                  try {
                    ...
                    logger.log(
                      `🔄 WebSocket token request: page=${page}, limit=${limit}, sortBy=${sortBy}, sortOrder=${sortOrder}`,
                    );
                    ...
                    const memoryCache = ((globalThis as any).__tokenCache =
                      (globalThis as any).__tokenCache || {});

                    // Track last request time for each client to prevent rapid duplicate requests
                    const clientRequestCache = ((
                      globalThis as any
                    ).__clientRequestTimes =
                      (globalThis as any).__clientRequestTimes || {});
                    const requesterId = server.__clientId || "unknown";
                    const now = Date.now();
                    const lastRequestTime =
                      clientRequestCache[`${requesterId}-${cacheKey}`] || 0;

                    // If this client has requested this exact data in the last second, throttle the request
                    if (now - lastRequestTime < 1000) {
                      logger.log(
                        `🚫 Throttling duplicate request from client ${requesterId} (${now - lastRequestTime}ms since last request)`,
                      );
                      // Still return cached data if available, but don't hit the API
                      if (
                        memoryCache[cacheKey] &&
                        now - memoryCache[cacheKey].timestamp < 60000
                      ) {
                        server.send(
                          JSON.stringify({
                            event: "tokensList",
                    ...
                    if (cachedItem && now - cachedItem.timestamp < 60000) {
                      // Use cached data
                      logger.log(
                        `✅ Using cached token data for ${cacheKey} (${now - cachedItem.timestamp}ms old)`,
                      );
                      server.send(
                        JSON.stringify({
                          event: "tokensList",
                    ...
                    Promise.race([
                      ...
                      .then((response) => {
                        ...
                        if (!(response as Response).ok) {
                          throw new Error(
                            `Token fetch failed: ${(response as Response).status}`,
                          );
                        }
                      ...
                      .then((tokenData) => {
                        ...
                        logger.log(
                          `✅ Sending token data via WebSocket: ${tokenData.tokens?.length || 0} tokens`,
                        );
                        server.send(
                          JSON.stringify({
                            event: "tokensList",
                      ...
                      .catch((error) => {
                        logger.error(
                          "❌ Error fetching tokens in dev WebSocket:",
                          error,
                        );
                    ...
                    logger.error(
                      "❌ Error handling token request in dev WebSocket:",
                      error,
                    );
                    server.send(
                      JSON.stringify({
                  ...
                  try {
                    ...
                    logger.log(
                      `🔍 WebSocket token search request: "${searchQuery}"`,
                    );
                    ...
                    // Use memory cache for development mode
                    const memoryCache = ((globalThis as any).__searchCache =
                      (globalThis as any).__searchCache || {});

                    // Track last request time for search requests
                    const clientRequestCache = ((
                      globalThis as any
                    ).__clientSearchTimes =
                      (globalThis as any).__clientSearchTimes || {});
                    const requesterId = server.__clientId || "unknown";
                    const now = Date.now();
                    const lastRequestTime =
                      clientRequestCache[`${requesterId}-${cacheKey}`] || 0;
                    ...
                    if (now - lastRequestTime < 1000) {
                      logger.log(
                        `🚫 Throttling duplicate search from client ${requesterId} (${now - lastRequestTime}ms since last search)`,
                      );
                      // Still return cached data if available, but don't hit the API
                      if (
                        memoryCache[cacheKey] &&
                    ...
                    if (cachedItem && now - cachedItem.timestamp < 60000) {
                      // Use cached data
                      logger.log(
                        `✅ Using cached search results for "${searchQuery}" (${now - cachedItem.timestamp}ms old)`,
                      );
                      server.send(
                        JSON.stringify({
                          event: "searchResults",
                    ...
                    // Not in cache or expired, fetch from API
                    logger.log(
                      `📥 Fetching fresh search results for "${searchQuery}"`,
                    );

                    // Construct API URL with search parameter
                    const searchUrl = new URL(
                      `${env.VITE_API_URL}/api/tokens/search`,
                    );
                    ...
                    Promise.race([
                      ...
                      .then((response) => {
                        ...
                        if (!(response as Response).ok) {
                          throw new Error(
                            `Search failed: ${(response as Response).status}`,
                          );
                        }
                        return (response as Response).json();
                      })
                      .then((searchData) => {
                        ...
                        // Send response to client
                        logger.log(
                          `✅ Sending search results via WebSocket: ${searchData.tokens?.length || 0} tokens found`,
                        );
                        server.send(
                          JSON.stringify({
                            event: "searchResults",
                      ...
                      .catch((error) => {
                        logger.error(
                          "❌ Error during token search in dev WebSocket:",
                          error,
                        );
                        ...
                        if (cachedItem) {
                          logger.log(
                            `⚠️ Using stale search cache due to fetch error`,
                          );
                          server.send(
                            JSON.stringify({
                              event: "searchResults",
                    ...
                    logger.error(
                      "❌ Error handling search request in dev WebSocket:",
                      error,
                    );
                    server.send(
                      JSON.stringify({
          ...
          server.addEventListener("close", () => {
            ...
            logger.log(
              `WebSocket client disconnected. Remaining: ${DEV_SOL_PRICE_CACHE.connectedClients.size}`,
            );

            // Clear interval if no more clients
            if (
              DEV_SOL_PRICE_CACHE.connectedClients.size === 0 &&
              solPriceUpdateInterval
            ) {
              clearInterval(solPriceUpdateInterval);
              solPriceUpdateInterval = null;
              logger.log(
                "No more WebSocket clients, stopped SOL price updates",
              );
            }
          });
