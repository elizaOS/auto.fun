...
class InMemoryWebSocketStore {
  ...
  broadcast(room: string, event: string, data: any, excludeClientId?: string): void {
    ...
    logger.log(`[LocalDev] Broadcasted ${event} to ${clients.size} clients in room ${room}`);
  }
...
api.post("/upload", async (c) => {
  try {
    ...
    if (!matches || matches.length !== 3) {
      ...
      return c.json({ error: "Invalid image format. Expected data URL format." }, 400);
    }
    ...
    logger.log(`Uploading image with content type: ${contentType}, filename: ${filename}`);
    ...
    logger.log(`Upload complete - Image: ${imageUrl}, Metadata: ${metadataUrl}`);
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === '/ws' && request.headers.get("Upgrade") === "websocket") {
      ...
      if ((env as any).WEBSOCKET_DO) {
        ...
        logger.log("Using in-memory WebSocket implementation for local development");
        ...
        try {
          ...
          const serverWebSocket = Object.assign(new WebSocket('ws://localhost'), {
            accept: () => {},
            readyState: WebSocket.OPEN
          ...
          return new Response("Failed to create WebSocket in local development", { 
            status: 500,
            headers: corsHeaders
          });
        }
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
    ...
    if (event.cron === "*/1 * * * *") { // Every minute
      ...
      await cron(env);
    } else if (event.cron === "*/15 * * * *") { // Every 15 minutes
