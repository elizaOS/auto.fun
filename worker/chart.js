...
import { fetchCodexBars, fetchCodexTokenEvents } from './codex';
import { SEED_BONDING_CURVE } from './constant';
import { fees, getDB, swaps, tokens } from './db';
...
export const DEV_TEST_TOKEN_ADDRESS = "ANNTWQsQ9J3PeM6dXLjdzwYcSzr51RREWQnjuuCEpump";
// Constants
...
export class TokenMonitor {
    wallet;
    isMonitoring = false;
    holderUpdateInterval;
    env;
    solanaConfig;
    constructor(env, wallet) {
        this.env = env;
        this.wallet = wallet;
        // Initialize Solana config with the environment
        this.solanaConfig = initSolanaConfig(env);
    }
    async startHolderUpdates() {
        // Clear any existing interval to avoid duplicates
        if (this.holderUpdateInterval) {
            clearInterval(this.holderUpdateInterval);
        }
        this.holderUpdateInterval = setInterval(async () => {
            try {
                const db = getDB(this.env);
                const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000).toISOString();
                // Get all active and migrated tokens
                const activeTokens = await db.select({ mint: tokens.mint })
                    .from(tokens)
                    .where(and(inArray(tokens.status, ['active', 'migrated', 'locked']), sql `${tokens.lastUpdated} < ${fiveMinutesAgo}` // Use SQL template literal for date comparison
                ));
                // Process tokens in small batches to avoid overloading the worker
                for (let i = 0; i < activeTokens.length; i += MAX_CONCURRENT_UPDATES) {
    ...
    async startMonitoring() {
        ...
        this.solanaConfig.connection.onLogs(this.solanaConfig.programId, async (logs) => {
            if (logs.err)
                return;
            // Look for swap logs
            ...
            if (completeEventLog) {
                try {
                    ...
                    const [bondingCurvePda] = PublicKey.findProgramAddressSync([Buffer.from(SEED_BONDING_CURVE), new PublicKey(mintAddress).toBytes()], this.solanaConfig.programId);
                    ...
                    for (let i = 0; i < maxRetries; i++) {
                        try {
                            ...
                            const bondingCurveAccountInfo = await this.solanaConfig.connection.getAccountInfo(bondingCurvePda);
                            ...
                            if (bondingCurveAccountInfo && bondingCurveAccountInfo.data) {
                                ...
                                if (token) {
                                    ...
                                    if (existingToken && existingToken.status &&
                                        ['migrating', 'withdrawn', 'migrated', 'locked'].includes(existingToken.status)) {
                                ...
                                if (i === maxRetries - 1) {
                                    logger.error('Failed to fetch bonding curve account after max retries');
                                    return;
                                }
                                await new Promise(resolve => setTimeout(resolve, 2000));
                                continue;
            ...
            if (newTokenLog) {
                const [tokenAddress, creatorAddress] = newTokenLog
                    ...
                    .map((s) => s.replace(/[",)]/g, ""));
                const db = getDB(this.env);
                const newToken = await createNewTokenData(logs.signature, tokenAddress, creatorAddress, this.env);
            ...
            if (mintLog || swapLog || reservesLog || feeLog) {
                if (!mintLog || !swapLog || !reservesLog || !feeLog) {
                    logger.error('Missing required logs:', { mintLog, swapLog, reservesLog, feeLog });
                    return;
                }
                try {
                    const db = getDB(this.env);
                    ...
                    if (!swapeventLog) {
                        logger.error('Missing swap event log:', { swapLog, reservesLog, feeLog });
                        return;
                    }
                    const [, , amountOut] = swapeventLog.split(" ").slice(-3).map((s) => s.replace(/[",)]/g, ''));
                    // Fetch token data to get decimals
                    const tokenMint = new PublicKey(mintAddress);
                    const tokenData = await getMint(this.solanaConfig.connection, tokenMint);
                    ...
                    const tokenSupply = this.env && this.env.TOKEN_SUPPLY ?
                    ...
                    if (!existingToken?.name) {
                        ...
                        const token = await createNewTokenData(tokenCreationTxId, mintAddress, creatorAddress);
                    ...
                    const tokenUpdate = {
                        ...
                        liquidity: (Number(reserveLamport) / 1e9 * solPrice) +
                            (Number(reserveToken) / Math.pow(10, TOKEN_DECIMALS) * tokenPriceUSD),
                        marketCapUSD: marketCapUSD,
                        tokenPriceUSD: tokenPriceUSD,
                        solPriceUSD: solPrice,
                        curveProgress: ((Number(reserveLamport) - virtualReserves) / (curveLimit - virtualReserves)) * 100,
                        lastUpdated: new Date().toISOString(),
                        priceChange24h: priceChange,
                    };
                    // Calculate volume24h increment
                    const volumeIncrement = direction === "1"
                    ...
                    const shouldUpdatePrice = !existingToken?.price24hAgo ||
                        (existingToken?.lastPriceUpdate && (Date.now() - new Date(existingToken.lastPriceUpdate).getTime() > ONE_HOUR));
                    ...
                    const shouldResetVolume = existingToken?.lastVolumeReset &&
                        (Date.now() - new Date(existingToken.lastVolumeReset).getTime() > ONE_DAY);
                    if (shouldResetVolume) {
                        ...
                        // Increment volume24h
                        tokenUpdate.volume24h = (existingToken?.volume24h || 0) + volumeIncrement;
                    }
                    ...
                    await db.update(tokens)
                        ...
                        ...(tokenUpdate.lastPriceUpdate ? { lastPriceUpdate: tokenUpdate.lastPriceUpdate } : {}),
                        ...(tokenUpdate.lastVolumeReset ? { lastVolumeReset: tokenUpdate.lastVolumeReset } : {})
                    })
                        .where(eq(tokens.mint, mintAddress));
                    // Create fee record
                    ...
                    const latestCandle = await getLatestCandle(this.env, mintAddress, swapData);
    ...
    async handleMigration(token) {
        // Use the adapter to create a compatible MigrationService
        const migrationService = createMigrationService(this.solanaConfig.connection, this.solanaConfig.programId, this.wallet, this.env);
        await migrationService.migrateToken(token);
    }
}
export async function getLatestCandle(env, tokenMint, swap) {
    ...
    const db = getDB(env);
    const [tokenInfo] = await db.select()
        .from(tokens)
    ...
    if (tokenInfo?.status === 'locked') {
        try {
            ...
            const tokenAddress = env.NETWORK === 'devnet'
                ? DEV_TEST_TOKEN_ADDRESS
                : tokenMint;
            const candles = await fetchCodexBars(tokenAddress, candleStart, candleStart + candlePeriod, '1' // 1 minute candles
            );
            if (candles.length > 0) {
                return candles[0];
            }
    ...
    const latestCandle = await fetchPriceChartData(env, candleStart * 1000, // start (ms)
...
export async function fetchPriceChartData(env, start, end, range, tokenMint) {
    const db = getDB(env);
    const [tokenInfo] = await db.select()
        .from(tokens)
        .where(eq(tokens.mint, tokenMint))
    ...
    if (tokenInfo.status !== 'locked') {
        ...
        const swapRecords = await db.select({
            price: swaps.price,
            ...
            .from(swaps)
            .where(and(eq(swaps.tokenMint, tokenMint), sql `${swaps.timestamp} >= ${new Date(start).toISOString()}`, sql `${swaps.timestamp} <= ${new Date(end).toISOString()}`))
            .orderBy(swaps.timestamp);
        // Convert to PriceFeedInfo array - ensure timestamp is not null
        const priceFeeds = swapRecords
        ...
        try {
            ...
            const candles = await fetchCodexBars(tokenAddress, Math.floor(start / 1000), Math.floor(end / 1000), resolution);
            ...
            try {
                ...
                const tokenEvents = await fetchCodexTokenEvents(tokenAddress, Math.floor(start / 1000), Math.floor(end / 1000));
                // Convert to price feed format - ensure timestamps are never null
                const priceFeeds = tokenEvents
...
export function getCandleData(priceFeeds, range) {
 ...
 */
export async function fetchLockedTokenChartData(token, start, end, range, _env) {
    try {
        ...
        const pairs = data.pairs.sort((a, b) => parseFloat(b.liquidity?.usd || '0') - parseFloat(a.liquidity?.usd || '0'));
        ...
        if (!chartData || !chartData.priceCandles || chartData.priceCandles.length === 0) {
 ...
 */
function groupCandlesByRange(candles, rangeMinutes) {
    if (candles.length === 0)
        return [];
    ...
    let currentGroup = [];
    let currentRangeStart = Math.floor(sortedCandles[0].time * 1000 / rangeMs) * rangeMs / 1000;
    // Process each candle
    for (const candle of sortedCandles) {
        const candleRangeStart = Math.floor(candle.time * 1000 / rangeMs) * rangeMs / 1000;
 ...
 */
function createCandleFromGroup(group, rangeStart) {
