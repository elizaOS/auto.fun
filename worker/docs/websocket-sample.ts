...
class AutoFunWebSocketClient {
  ...
  constructor(private apiUrl: string, private clientId?: string) {
    this.connect();
  }
  ...
  private handleMessage(event: MessageEvent): void {
    try {
      ...
      if (message.event === 'subscribed') {
        console.log(`Subscribed to token: ${message.data.room.replace('token-', '')}`);
        return;
      }
  ...
  private handleClose(event: CloseEvent): void {
    ...
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      const delay = Math.min(this.reconnectTimeout * this.reconnectAttempts, 30000); // Max 30 seconds
