 ...
 */
export async function createCharacterDetails(request, env) {
    try {
...
${name} continues to learn and improve with each interaction.`,
        ...
        if (env && 'OPENAI_API_KEY' in env && env.OPENAI_API_KEY && inputs.model) {
            try {
                const aiGenerated = await generateWithAI(request, env.OPENAI_API_KEY);
 ...
 */
async function generateWithAI(request, apiKey) {
    const { inputs, requestedOutputs } = request;
 ...
 */
function parseAIResponse(response, requestedOutputs) {
    const result = {};
...
export async function getAllPersonalities(env) {
    try {
        const db = getDB(env);
 ...
 */
export async function createPersonality(name, description, env) {
    const db = getDB(env);
