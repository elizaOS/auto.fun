 ...
 */
export async function getSOLPrice(env) {
 ...
 */
export async function fetchSOLPriceFromPyth() {
    try {
 ...
 */
export async function updateSOLPrice(env) {
    try {
 ...
 */
export async function calculateTokenMarketData(token, solPrice) {
    ...
    try {
        ...
        if (token.reserveAmount && token.tokenPriceUSD) {
            tokenWithMarketData.marketCapUSD = token.reserveAmount * token.tokenPriceUSD;
        }
        // Add SOL price
...
async function calculateRaydiumTokenMarketData(token, env) {
    try {
        ...
        while (retries > 0) {
            try {
                ...
                retries--;
                if (retries === 0) {
                    logger.error(`Mcap: Failed to fetch pool info after retries: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
        ...
        if (typeof poolInfo.mintAmountA !== 'number' || typeof poolInfo.mintAmountB !== 'number') {
            throw new Error('Mcap: Invalid pool info structure');
        }
        ...
        const marketCapUSD = env ?
            (Number(env.TOKEN_SUPPLY) / Math.pow(10, TOKEN_DECIMALS)) * tokenPriceUSD :
 ...
 */
export async function updateMigratedTokenMarketData(env) {
    ...
    try {
        ...
        }; // Cast to Env for compatibility
        const db = getDB(workingEnv);
        const migratedTokens = await db.select()
            .from(tokens)
        ...
        for (let i = 0; i < migratedTokens.length; i += MAX_CONCURRENT_TOKENS) {
            ...
            const batchResults = await Promise.all(tokenBatch.map(async (token) => {
                try {
