...
// Import blockchain utils from the correct path
import { fetchTokenTransactions, getConnection } from "../../src/utils/blockchain";
...
tokenRouter.get("/api/swaps/:mint", async (c) => {
  ...
  try {
    ...
    if (swapCount === 0 && c.env.NODE_ENV === "development") {
      ...
      // Try to update from blockchain if no data in DB
  ...
  try {
    ...
    try {
      ...
      const formattedSwaps = swapRecords.slice(0, 5).map((swap) => {
        ...
        const solAmount =
          (swap.direction === 0 ? swap.amountIn ?? 0 : swap.amountOut ?? 0) / 1e9; // Add null check
        const tokenAmount =
          (swap.direction === 0 ? swap.amountOut ?? 0 : swap.amountIn ?? 0) / 1e6; // Add null check
      ...
      // Emit each swap event individually to the global room
      for (const swap of formattedSwaps) {
        await socket.to("global").emit("newSwap", swap); // Use .to("global").emit()
        logger.log(`Emitted global newSwap event for txId ${swap.txId}`);
      }
...
tokenRouter.get("/direct-swaps/:mint", async (c) => {
  try {
    ...
    const formattedSwaps = txResult.swaps.map((swap) => ({
      ...
      tokenAmount:
        (swap.direction === 0 ? swap.amountOut : swap.amountIn) / 1e6,
    }));
...
// --- Callback endpoint for external vanity keypair grinder ---
tokenRouter.post("/internal/vanity-callback", async (c) => {
  logger.log("[/internal/vanity-callback] Received callback from vanity grinder");
  try {
    // TODO: Add security check (e.g., shared secret header/query param)
    // const secret = c.req.header("X-Grinder-Secret") || c.req.query("secret");
    // if (secret !== c.env.GRINDER_CALLBACK_SECRET) {
    //   logger.warn("[/internal/vanity-callback] Invalid or missing secret");
    //   return c.json({ error: "Unauthorized" }, 401);
    // }

    const job = await c.req.json();
    logger.log("[/internal/vanity-callback] Received job data:", {
      jobId: job.id,
      status: job.status,
      hasResult: !!job.result,
      address: job.result?.address,
      hasKeypairBytes: Array.isArray(job.result?.keypair_bytes),
      keypairBytesLength: job.result?.keypair_bytes?.length,
    });

    if (job.status !== "Completed" || !job.result) {
      logger.warn(
        `[/internal/vanity-callback] Job not completed or missing result. Status: ${job.status}`,
      );
      return c.json({ message: "Acknowledged non-completed job" }, 200);
    }
    ...
    if (
      ...
      !keypair_bytes ||
      !Array.isArray(keypair_bytes) ||
      keypair_bytes.length !== 64
    ) {
      logger.error(
        "[/internal/vanity-callback] Invalid data received from grinder:",
        {
          address: typeof address,
          keypair_bytes: typeof keypair_bytes,
          isArray: Array.isArray(keypair_bytes),
          length: keypair_bytes?.length,
        },
      );
      return c.json({ error: "Invalid keypair data format" }, 400);
    }

    // Convert keypair_bytes (number[]) to Uint8Array, then to base64
    let secretKeyBase64: string;
    try {
      const secretKeyUint8Array = new Uint8Array(keypair_bytes);
      secretKeyBase64 = Buffer.from(secretKeyUint8Array).toString("base64");
    } catch (conversionError) {
      logger.error(
        "[/internal/vanity-callback] Error converting keypair bytes to base64:",
        conversionError,
      );
      return c.json({ error: "Failed to process keypair data" }, 500);
    }

    // Prepare for database insertion
    const db = getDB(c.env);
    const newKeypair: VanityKeypairInsert = {
      id: crypto.randomUUID(),
      address: address,
      secretKey: secretKeyBase64,
      createdAt: new Date().toISOString(),
      used: 0, // Mark as unused
    };
    ...
    try {
      await db.insert(vanityKeypairs).values(newKeypair);
      logger.log(
        `[/internal/vanity-callback] Successfully inserted new vanity keypair: ${address}`,
      );
    } catch (dbError) {
      if (
        dbError instanceof Error &&
        ...
        logger.warn(
          `[/internal/vanity-callback] Vanity keypair ${address} already exists in DB. Callback likely duplicated.`,
        );
        // Still return 200 OK to acknowledge the callback
      } else {
        logger.error(
          "[/internal/vanity-callback] Database error inserting keypair:",
          dbError,
        );
        // Return 500 but grinder might retry, maybe 200 is safer?
        // Let's return 200 to prevent grinder retries for DB errors.
        return c.json(
          { message: "Acknowledged but failed to save keypair" },
          200,
        );
      }
    }
    ...
    // Acknowledge receipt
    return c.json({ message: "Keypair received and processed" }, 200);
  } catch (error) {
    logger.error("[/internal/vanity-callback] Unexpected error:", error);
    // Acknowledge receipt even on unexpected errors to prevent grinder retries
    return c.json({ message: "Acknowledged but failed unexpectedly" }, 200);
  }
});
