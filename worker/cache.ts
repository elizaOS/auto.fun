...
export class CacheService {
   ...
   */
  async setTokenPrice(mint: string, price: number, ttlSeconds: number = 300): Promise<void> {
    try {
      const now = new Date();
      const expiresAt = new Date(now.getTime() + ttlSeconds * 1000).toISOString();
   ...
   */
  async setMetadata(key: string, data: any, ttlSeconds: number = 3600): Promise<void> {
    try {
      const now = new Date();
      const expiresAt = new Date(now.getTime() + ttlSeconds * 1000).toISOString();
   ...
   */
  private async cleanupOldCacheEntries(type: string, symbol: string): Promise<void> {
    try {
      const now = new Date().toISOString();
