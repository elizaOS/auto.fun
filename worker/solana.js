...
const createWallet = (env) => {
    ...
    if (env?.WALLET_PRIVATE_KEY) {
        try {
            const keypair = Keypair.fromSecretKey(Uint8Array.from(JSON.parse(env.WALLET_PRIVATE_KEY)), { skipValidation: true });
    ...
    if (process.env.WALLET_PRIVATE_KEY) {
        try {
            const keypair = Keypair.fromSecretKey(Uint8Array.from(JSON.parse(process.env.WALLET_PRIVATE_KEY)), { skipValidation: true });
            return new NodeWallet(keypair);
        }
        catch (error) {
...
export const initSolanaConfig = (env) => {
    try {
        return {
        ...
        throw new Error("Failed to initialize Solana config:", error);
    }
};
