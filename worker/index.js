...
api.post('/message-likes/:messageId', async (c) => {
    try {
        ...
        const existingLike = await db.select()
            .from(messageLikes)
            .where(and(eq(messageLikes.messageId, messageId), eq(messageLikes.userAddress, userAddress)))
            .limit(1);
...
async function addHasLikedToMessages(db, messagesList, userAddress) {
    ...
    const userLikes = await db.select()
        .from(messageLikes)
        .where(and(inArray(messageLikes.messageId, messageIds), eq(messageLikes.userAddress, userAddress)));
...
api.post('/agent-details', async (c) => {
    try {
        ...
        const invalidOutputs = requestedOutputs.filter((output) => !allowedOutputs.includes(output));
...
api.get('/agents', async (c) => {
    try {
        ...
        const agentsList = await db.select({
            ...
            .from(agents)
            .where(and(eq(agents.ownerAddress, ownerAddress), sql `agents.deletedAt IS NULL`))
...
api.get('/agents/:id', async (c) => {
    try {
        ...
        const agent = await db.select()
            .from(agents)
            .where(and(eq(agents.id, id), eq(agents.ownerAddress, user.publicKey), sql `agents.deletedAt IS NULL`))
            .limit(1);
...
api.get('/agents/mint/:contractAddress', async (c) => {
    try {
        ...
        const agent = await db.select()
            .from(agents)
            .where(and(eq(agents.contractAddress, contractAddress), eq(agents.ownerAddress, user.publicKey), sql `agents.deletedAt IS NULL`))
            .limit(1);
        if (!agent || agent.length === 0) {
            return c.json({ error: 'Agent not found or unauthorized' }, 404);
        }
...
function isValidAdminKey(c, apiKey) {
    if (!apiKey)
        return false;
