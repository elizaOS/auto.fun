...
export async function processTransactionLogs(
  ...
  try {
    ...
    let result: {found: boolean, tokenAddress?: string, event?: string} = {found: false};
    ...
    const completeEventLog = logs.find(log => log.includes("curve is completed"));
    ...
    if (newTokenLog) {
      try {
        ...
        const isValidTokenAddress = /^[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]+$/.test(rawTokenAddress);
        const isValidCreatorAddress = /^[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]+$/.test(rawCreatorAddress);
        
        if (!isValidTokenAddress || !isValidCreatorAddress) {
          logger.error(`Invalid address format in NewToken log: token=${rawTokenAddress}, creator=${rawCreatorAddress}`);
          return {found: false};
        }
        ...
        result = {found: true, tokenAddress: rawTokenAddress, event: 'newToken'};
      } catch (error) {
    ...
    if (mintLog && swapLog && reservesLog && feeLog) {
      try {
        ...
        try {
          ...
          if (!mintAddress || !/^[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]+$/.test(mintAddress)) {
        ...
        try {
          ...
          if (!user || !direction || !amount) {
            logger.error(`Missing swap data: user=${user}, direction=${direction}, amount=${amount}`);
            return result;
          }
        ...
        try {
          ...
          reserveToken = reservesParts[reservesParts.length - 2].replace(/[",)]/g, '');
          reserveLamport = reservesParts[reservesParts.length - 1].replace(/[",)]/g, '');
          
          // Validate extracted data
          if (!reserveToken || !reserveLamport) {
            logger.error(`Missing reserves data: reserveToken=${reserveToken}, reserveLamport=${reserveLamport}`);
            return result;
          }
          
          // Make sure reserve values are numeric
          if (isNaN(Number(reserveToken)) || isNaN(Number(reserveLamport))) {
            logger.error(`Invalid reserve values: reserveToken=${reserveToken}, reserveLamport=${reserveLamport}`);
            return result;
          }
        } catch (error) {
        ...
        const tokenAmountDecimal = Number(reserveToken) / Math.pow(10, TOKEN_DECIMALS);
        ...
        logger.log(`Saved swap: ${direction === "0" ? "buy" : "sell"} for ${mintAddress}`);
    ...
    if (completeEventLog && mintLog) {
      try {
        ...
        try {
          ...
          // Validate mint address format
          if (!mintAddress || !/^[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]+$/.test(mintAddress)) {
            logger.error(`Invalid mint address format in curve completion: ${mintAddress}`);
            return result;
          }
        } catch (error) {
          logger.error(`Error parsing mint address in curve completion: ${error}`);
          return result;
        }
        ...
        result = {found: true, tokenAddress: mintAddress, event: 'curveComplete'};
      } catch (error) {
...
export async function monitorSpecificToken(env: Env, tokenMint: string): Promise<{found: boolean, message: string}> {
  logger.log(`Looking for specific token: ${tokenMint}`);
  
  try {
    const wsClient = getWebSocketClient(env);
    const connection = new Connection((env.NETWORK === "devnet" ? env.DEVNET_SOLANA_RPC_URL : env.MAINNET_SOLANA_RPC_URL) || "https://api.mainnet-beta.solana.com");
    ...
    try {
      ...
      return {found: false, message: `Invalid program ID: ${error instanceof Error ? error.message : "Unknown error"}`};
    }
    ...
    const isValidBase58 = /^[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]+$/.test(tokenMint);
    ...
    if (isValidBase58) {
      try {
        ...
        logger.log(`Could not get signatures for token ${tokenMint}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        logger.log(`Falling back to checking program signatures only`);
      }
    } else {
      logger.log(`Token ${tokenMint} contains invalid base58 characters, skipping direct token lookup`);
    }
    ...
    if (signatures.length === 0) {
      ...
      try {
        logger.log(`No signatures found, but creating basic token record for ${tokenMint}`);
        ...
        return {found: true, message: "Token added to database but details will be updated later"};
      } catch (error) {
        logger.error(`Error creating basic token record:`, error);
        return {found: false, message: "No transactions found and could not create token record"};
      }
    }
    ...
    for (const signatureInfo of signatures) {
      try {
        ...
        if (relevantLogs.length > 0) {
          logger.log(`Found ${relevantLogs.length} relevant logs for ${tokenMint} in tx ${signatureInfo.signature}`);
          
          try {
            // Process the transaction logs
            const result = await processTransactionLogs(env, logs, signatureInfo.signature, wsClient);
            ...
            if (result.found) {
              if (result.tokenAddress === tokenMint) {
                logger.log(`Successfully processed token ${tokenMint} from transaction ${signatureInfo.signature}`);
                return {found: true, message: `Token found and processed (${result.event})`};
              } else {
                logger.log(`Found a token in transaction, but not the one we're looking for. Found ${result.tokenAddress} vs ${tokenMint}`);
              }
            }
          } catch (error) {
            logger.error(`Error processing logs for transaction ${signatureInfo.signature}:`, error);
          }
        }
      } catch (txError) {
        logger.error(`Error fetching transaction ${signatureInfo.signature}:`, txError);
      }
    }
    ...
    try {
      logger.log(`No matching transaction found, but creating basic token record for ${tokenMint}`);
      ...
      logger.error(`Error creating basic token record:`, error);
      return {found: false, message: "Could not create token record after searching transactions"};
    }
  } catch (error) {
...
export async function monitorTokenEvents(env: Env): Promise<void> {
  try {
    ...
    const isValidBase58 = /^[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]+$/.test(env.PROGRAM_ID);
    if (!isValidBase58) {
      logger.error(`Invalid PROGRAM_ID format: ${env.PROGRAM_ID} - contains non-base58 characters`);
      return;
    }
    ...
    try {
      ...
      logger.error(`Invalid PROGRAM_ID: ${env.PROGRAM_ID} - ${error instanceof Error ? error.message : "Unknown error"}`);
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < signatures.length; i++) {
        ...
        try {
          ...
          logger.error(`Error processing transaction ${signatureInfo.signature}:`, txError);
      ...
      logger.error(`Error getting signatures for program ${env.PROGRAM_ID}:`, sigError);
    }
  } catch (error) {
    ...
    for (const token of activeTokens) {
      try {
        if (token.mint) {
          ...
          logger.log(`Updated holders for ${token.mint}: ${holderCount} holders`);
        }
      } catch (err) {
