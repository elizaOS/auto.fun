...
let _fileCache: {[key: string]: string} = {};
...
export async function uploadToCloudflare(
  ...
  const contentType =
    options.contentType ||
    (options.isJson ? "application/json" : "image/jpeg");
    
  logger.log(`Using content type ${contentType} for upload, filename: ${options.filename || 'none'}`);
  ...
  try {
    ...
    if (isLocalDev) {
      ...
      const localDevBaseUrl = 'http://localhost:8787/api/direct-file';
      ...
      // Generate local dev URL that uses our direct-file endpoint
    ...
    try {
      ...
      if (!baseUrl) {
        logger.warn("R2_PUBLIC_URL environment variable is not set. Using default URL format.");
