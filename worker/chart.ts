...
import { CodexBarResolution, fetchCodexBars, fetchCodexTokenEvents } from './codex';
      ...
      async (logs: { err: any; logs: any[]; signature: string; }) => {
        ...
        if (mintLog || swapLog || reservesLog || feeLog) {
          ...
          try {
            ...
            // Parse logs
            const mintAddress = mintLog.split("Mint:")[1].trim().replace(/[",)]/g, '');
            const [user, direction, amount] = swapLog.split(" ").slice(-3).map((s: string) => s.replace(/[",)]/g, ''));
            const [reserveToken, reserveLamport] = reservesLog.split(" ").slice(-2).map((s: string) => s.replace(/[",)]/g, ''));
            const feeAmount = feeLog.split("fee:")[1].trim().replace(/[",)]/g, '');
            if(!swapeventLog) {
              logger.error('Missing swap event log:', { swapLog, reservesLog, feeLog });
              return;
            }
            const [, , amountOut] = swapeventLog.split(" ").slice(-3).map((s: string) => s.replace(/[",)]/g, ''));
...
export async function fetchPriceChartData(env: Env, start: number, end: number, range: number, tokenMint: string) {
  const db = getDB(env);
  const [tokenInfo] = await db.select()
    .from(tokens)
    .where(eq(tokens.mint, tokenMint))
 ...
 */
function groupCandlesByRange(candles: Candle[], rangeMinutes: number): Candle[] {
