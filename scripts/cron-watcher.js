...
const LOCAL_ENDPOINT = `http://localhost:${port}/__scheduled`;
...
async function triggerCron() {
  try {
    ...
    if (response.ok) {
      ...
      console.error(`[${timestamp}] ❌ Failed to trigger cron: ${response.status}`);
      const text = await response.text();
      console.error(`Response: ${text.substring(0, 200)}${text.length > 200 ? '...' : ''}`);
    }
  } catch (error) {
