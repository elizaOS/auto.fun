...
const setupHeliusWebhook = async () => {
  ...
  const rpcUrl = process.env.VITE_DEVNET_RPC_URL;
  const heliusApiKey = extractHeliusApiKey(rpcUrl);
  
  if (!heliusApiKey) {
    console.error('❌ Could not extract Helius API key from VITE_DEVNET_RPC_URL');
    console.error('Please ensure your .env file contains a valid Helius RPC URL');
    process.exit(1);
  }
  ...
  try {
    ...
    let data;
    if (existingWebhook) {
      data = await updateWebhook(heliusApiKey, existingWebhook.webhookID, webhookUrl);
    } else {
      data = await createWebhook(heliusApiKey, webhookUrl);
    }
    
    console.log(`Webhook ID: ${data.webhookID}`);
    console.log('🎉 Setup complete! The tunnel will remain active for development.');
