...
import { TokenHolder, TokenMarketMetrics, TokenTransaction } from "./blockchain";
...
export function useTokenDataWS(tokenMint: string | null | undefined) {
  ...
  const fetchToken = useCallback((bypassCache: boolean = false) => {
    if (!tokenMint || !wsConnected || !sendMessage) {
      return false;
    }
...
export function useTokenBalanceWS(
  ...
  useEffect(() => {
    ...
    const unsubscribe = addEventListener<{
      ...
      if (response.address !== walletAddress || response.mint !== tokenMint) return;
...
export function useWalletTokenBalanceWS(
  ...
  return {
    ...
    lastUpdate: Math.max(
      solBalance.lastUpdate || 0,
      tokenBalance.lastUpdate || 0
    ) || null,
    refresh: refreshAll
  };
} 