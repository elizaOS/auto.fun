...
export const getRpcUrl = (): string => {
  ...
  return env.mainnetRpcUrl || "https://mainnet.helius-rpc.com/?api-key=28741947-d621-41a0-95db-bbd1b57ccf8a";
};
  ...
  private async processQueue() {
    ...
    try {
      ...
      if (error.message?.includes('429') || error.toString().includes('429')) {
        console.log('Rate limit hit (429). Adding request back to queue with backoff.');
...
export const fetchTokenMarketMetrics = async (tokenMint: string): Promise<TokenMarketMetrics> => {
  try {
    console.log(`Fetching token market metrics from blockchain for ${tokenMint}`);
    ...
    const signatures = await queuedRequest(
      ...
      cache.signatures,
      () => connection.getSignaturesForAddress(new PublicKey(tokenMint), { limit: 25 }),
      60000 // 1 minute cache for signatures
    );

    console.log(`Found ${signatures.length} signatures to analyze for price discovery`);
    ...
    if (signatures.length > 0) {
      ...
      const txPromises = signatures.slice(0, 10).map(async (sig) => {
        try {
          ...
          if (tx.meta.preBalances && tx.meta.postBalances) {
            ...
            if (tx.meta.preBalances.length > feePayerIndex && tx.meta.postBalances.length > feePayerIndex) {
            ...
            for (let i = 0; i < tx.meta.preBalances.length; i++) {
              ...
              if (Math.abs(balanceChange) > 0.001) {
                if ((isBuy && balanceChange > 0) || (!isBuy && balanceChange < 0)) {
          ...
          if (solAmount > solFees && tokenAmount > 0) {
            ...
            console.log(`Found swap: ${isBuy ? 'BUY' : 'SELL'}, Token Amount: ${tokenAmount}, SOL Amount: ${actualSolAmount}, Price: ${price}`);
      ...
      console.log(`Found ${validTxs.length} valid swap transactions for price calculation`);
      ...
      if (validTxs.length > 0) {
        ...
        console.log(`Market cap: $${marketCapUSD} (Total Supply: ${totalSupply})`);
        ...
        volume24h = txs24h.reduce((sum, tx) => sum + (tx.tokenAmount * tx.price * solPriceUSD), 0);
    ...
    console.error(`Error fetching token market metrics from blockchain:`, error);
...
export const fetchTokenHolders = async (tokenMint: string): Promise<{ holders: TokenHolder[], total: number }> => {
  try {
    console.log(`Fetching token holders directly from blockchain for ${tokenMint}`);
    ...
    const holdersWithPercentage = holders.map(holder => ({
      ...
      percentage: totalTokens > 0 ? (holder.raw / totalTokens * 100).toFixed(2) : '0.00'
    }));
...
export const fetchTokenTransactions = async (tokenMint: string, limit = 10): Promise<{ swaps: TokenTransaction[], total: number }> => {
  try {
    ...
    console.log(`Fetching token transactions directly from blockchain for ${tokenMint}`);
    const connection = getConnection();
    ...
    console.log(`Found ${signatures.length} recent signatures for mint ${tokenMint}`);
    ...
    const signaturesToProcess = signatures.slice(0, Math.min(signatures.length, 5));
    ...
    const validTransactions = transactions.filter(tx => tx !== null) as TokenTransaction[];
...
export const fetchTokenChartData = async (
  ...
  try {
    ...
    console.log(`Fetching token chart data from blockchain for ${tokenMint} (resolution: ${resolution})`);
    const connection = getConnection();
    ...
    console.log(`Filtered to ${filteredSignatures.length} signatures in the time range`);
    
    // Process a reasonable batch of signatures to get good chart data
    const signaturesToProcess = filteredSignatures.slice(0, Math.min(filteredSignatures.length, 25));
    ...
    const pricePoints = (await Promise.all(transactionPromises))
    ...
    console.log(`Found ${pricePoints.length} valid price points for chart data`);
...
function generateCandles(
  pricePoints: { time: number, price: number, solAmount?: number, tokenAmount?: number }[],
  resolution: number,
  from: number,
  to: number
): TokenPriceCandle[] {
  ...
  let currentTime = Math.floor(pricePoints[0].time / resolutionInSeconds) * resolutionInSeconds;
  
  while (currentTime <= to) {
    const periodPoints = pricePoints.filter(p => 
