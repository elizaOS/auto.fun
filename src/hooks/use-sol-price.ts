...
const fetchSolPrice = async (): Promise<number> => {
  ...
  const now = Date.now();
  if (
    GLOBAL_SOL_PRICE_STATE.cachedPrice !== null &&
    now - GLOBAL_SOL_PRICE_STATE.lastFetchTime <
      GLOBAL_SOL_PRICE_STATE.CACHE_VALIDITY
  ) {
  ...
  if (GLOBAL_SOL_PRICE_STATE.isInitialLoad) {
    console.log(
      "Initial app load - skipping HTTP fetch and using default price while WebSocket connects",
    );
...
export function useSolPrice() {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      if (connected) {
        console.log(
          "SOL Price: WebSocket connected, trying to get price via WebSocket",
        );
        ...
        if (cachedWsPrice !== null) {
          console.log(
            "SOL Price: Using last known WebSocket price:",
            cachedWsPrice,
          );
        ...
        if (wsPrice !== null) {
          console.log(
            "SOL Price: Got immediate response from WebSocket:",
            wsPrice,
          );
          // Update our global cache
        ...
        console.log(
          `SOL Price: Waiting up to ${wsTimeout}ms for WebSocket response...`,
        );
        ...
        try {
          const price = await new Promise<number>((resolve, reject) => {
            ...
            const unsubscribe = addEventListener<{ price: number }>(
              "solPriceUpdate",
              (data) => {
                if (data && data.price) {
          ...
          console.log(
            "SOL Price: Successfully received price via WebSocket:",
            price,
          );
          // Update our global cache
          GLOBAL_SOL_PRICE_STATE.cachedPrice = price;
          ...
          console.log(
            "SOL Price: WebSocket request timed out, falling back to HTTP",
          );
      ...
      if (GLOBAL_SOL_PRICE_STATE.isInitialLoad) {
        console.log(
          "Initial app load - returning default price while waiting for WebSocket",
        );
        return GLOBAL_SOL_PRICE_STATE.DEFAULT_PRICE;
      }
      ...
      if (
        connected &&
        now - GLOBAL_SOL_PRICE_STATE.lastWebSocketUpdate <
          GLOBAL_SOL_PRICE_STATE.WS_COOLDOWN_PERIOD
      ) {
        console.log(
          "SOL Price: Recent WebSocket update available, skipping HTTP fetch",
        );
  ...
  useEffect(() => {
    console.log(
      "SOL Price: Setting up WebSocket listeners, connected:",
      connected,
    );
    ...
    const unsubscribe = addEventListener<{ price: number }>(
      ...
      (data) => {
        if (data && data.price) {
          const price = Number(data.price);
          console.log(
            "SOL Price: Received SOL price update via WebSocket:",
            price,
          );
  ...
  useEffect(() => {
    if (connected) {
      console.log(
        "SOL Price: WebSocket connected, setting up periodic price requests",
      );
      ...
      console.log(
        "SOL Price: WebSocket disconnected, relying on HTTP fallback",
      );
