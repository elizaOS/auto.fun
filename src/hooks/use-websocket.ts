  ...
  if (GLOBAL_STATE.isConnected !== connected) {
    ...
    GLOBAL_STATE.connectionStateListeners.forEach((listener) => {
      try {
        ...
        console.error(
          "WebSocket: Error notifying connection state listener:",
          error,
        );
      }
    });
...
const initializeSocket = () => {
  ...
  if (GLOBAL_STATE.connectionInProgress || GLOBAL_STATE.connectionCooldown) {
    console.log(
      `WebSocket: Connection attempt skipped - in progress: ${GLOBAL_STATE.connectionInProgress}, cooldown: ${GLOBAL_STATE.connectionCooldown}`,
    );
    return;
  }
...
const handleMessage = (event: MessageEvent) => {
  try {
    ...
    const now = Date.now();
    const lastTimestamp =
      GLOBAL_STATE.lastMessageTimestamps.get(messageKey) || 0;
    ...
    if (eventType === "solPriceUpdate" && data.data && data.data.price) {
      ...
      console.log(
        `WebSocket: Received SOL price update: $${priceValue} (clearing pending flag)`,
      );
      ...
      if (now - GLOBAL_STATE.solPriceRequest.lastRequest > fiveMinutes) {
        console.log(
          "WebSocket: SOL price update received after long delay, requesting fresh price",
        );
    ...
    if (eventType === "tokensList" && data.data && data.data.page) {
      ...
      for (const [_, request] of GLOBAL_STATE.pendingTokenRequests.entries()) {
        if (request.page === page) {
          console.log(
            `WebSocket: Received token list for page ${page} with ${limit} tokens`,
          );
          // Mark as received
      ...
      if (GLOBAL_AUTH_STATE) {
        console.log(
          "WebSocket: Setting tokensReceivedViaWebSocket flag to true",
        );
    ...
    if (
      eventType &&
      GLOBAL_STATE.eventHandlers.has(eventType as WebSocketEvent)
    ) {
      const handlers = GLOBAL_STATE.eventHandlers.get(
        eventType as WebSocketEvent,
      );
...
const handleClose = (event: CloseEvent) => {
  ...
  if (
    GLOBAL_STATE.instanceCount > 0 &&
    GLOBAL_STATE.reconnectAttempts < MAX_RECONNECT_ATTEMPTS
  ) {
    GLOBAL_STATE.reconnectAttempts++;
    const delay = Math.min(
      1000 * Math.pow(2, GLOBAL_STATE.reconnectAttempts),
      30000,
    );
    console.log(
      `WebSocket: Reconnecting in ${delay / 1000}s (attempt ${GLOBAL_STATE.reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})...`,
    );

    GLOBAL_STATE.reconnectTimeout = setTimeout(() => {
      console.log(
        `WebSocket: Attempting reconnect #${GLOBAL_STATE.reconnectAttempts}...`,
      );
      initializeSocket();
    }, delay);
...
const resubscribe = () => {
  console.log(
    `WebSocket: Resubscribing to ${GLOBAL_STATE.tokenSubscriptions.size} tokens and global=${GLOBAL_STATE.isGlobalSubscribed}`,
  );
...
const requestSolPriceInternal = (): boolean => {
  ...
  if (
    now - GLOBAL_STATE.solPriceRequest.lastRequest <
    GLOBAL_STATE.solPriceRequest.cooldownPeriod
  ) {
    console.log(
      `WebSocket: SOL price request in cooldown (${Math.round((now - GLOBAL_STATE.solPriceRequest.lastRequest) / 1000)}s elapsed of ${Math.round(GLOBAL_STATE.solPriceRequest.cooldownPeriod / 1000)}s cooldown)`,
    );
    return false;
  }
  ...
  if (GLOBAL_STATE.socket?.readyState === WebSocket.OPEN) {
    try {
      ...
      setTimeout(() => {
        if (GLOBAL_STATE.solPriceRequest.pendingRequest) {
          console.log(
            "WebSocket: SOL price request timed out, clearing pending flag",
          );
          GLOBAL_STATE.solPriceRequest.pendingRequest = false;
        }
    ...
    console.warn(
      `WebSocket: Cannot request SOL price, not connected (readyState: ${GLOBAL_STATE.socket?.readyState})`,
    );
    GLOBAL_STATE.solPriceRequest.pendingRequest = false;
    ...
    if (GLOBAL_STATE.isConnected) {
      ...
      // Try to reconnect
      if (
        !GLOBAL_STATE.connectionInProgress &&
        !GLOBAL_STATE.connectionCooldown
      ) {
        setTimeout(() => {
          console.log(
            "WebSocket: Reconnecting after connection state mismatch in SOL price request",
          );
          initializeSocket();
        }, 1000);
      }
...
const canSendMessages = () => {
  return (
    GLOBAL_STATE.socket && GLOBAL_STATE.socket.readyState === WebSocket.OPEN
  );
};
...
const sendMessageWithDeduplication = (message: any): boolean => {
  ...
  if (canSendMessages()) {
    ...
    console.warn(
      `WebSocket: Cannot send message, socket not open (readyState: ${GLOBAL_STATE.socket?.readyState})`,
    );
    ...
    if (GLOBAL_STATE.isConnected) {
      ...
      if (
        ...
        setTimeout(() => {
          console.log(
            "WebSocket: Reconnecting after connection state mismatch",
          );
          initializeSocket();
        }, 1000);
      }
...
const requestTokenList = (
  ...
  if (!canSendMessages()) {
    console.log(
      "WebSocket: Cannot request tokens - not connected or socket not ready",
    );

    // Check for connection state mismatch and attempt to fix
    if (
      GLOBAL_STATE.isConnected &&
      GLOBAL_STATE.socket &&
      GLOBAL_STATE.socket.readyState !== WebSocket.OPEN
    ) {
      console.warn(
        "WebSocket: Connection state mismatch detected in requestTokenList - fixing",
      );
      updateConnectionState(false);
      ...
      if (
        ...
        setTimeout(() => {
          console.log(
            "WebSocket: Reconnecting due to state mismatch in requestTokenList",
          );
          if (GLOBAL_STATE.socket) {
            try {
              GLOBAL_STATE.socket.close();
  ...
  try {
    ...
    if (success) {
      console.log(
        `WebSocket: Requested token list for page ${page} (${sortBy}, ${sortOrder})`,
      );
    } else {
...
const isTokenRequestActive = (
  ...
  for (const [key, request] of GLOBAL_STATE.pendingTokenRequests.entries()) {
    if (
      key.startsWith(requestKeyPattern) &&
...
export function useWebSocket() {
  ...
  const instanceIdRef = useRef<string>(
    `ws-${Math.random().toString(36).substring(2, 11)}`,
  );
  ...
  useEffect(() => {
    ...
    if (MOUNTED_COMPONENTS.has(instanceId)) {
      console.log(
        `WebSocket hook already initialized for instance ${instanceId}`,
      );
      return;
    }
    ...
    console.log(
      `WebSocket hook instance created (${instanceId}). Total instances: ${GLOBAL_STATE.instanceCount}`,
    );

    // Use a very short delay (or no delay) for first instance to prioritize WebSocket connection
    const isFirstInstance =
      GLOBAL_STATE.instanceCount === 1 || MOUNTED_COMPONENTS.size === 1;
    ...
    if (
      ...
      setTimeout(() => {
        ...
        if (!GLOBAL_STATE.socket && !GLOBAL_STATE.connectionInProgress) {
          console.log(
            `Init WebSocket from instance ${instanceId}${isFirstInstance ? " (first instance)" : ""} after ${delay}ms delay`,
          );
          initializeSocket();
        } else {
          console.log(
            `Connection already in progress, skipping init from ${instanceId}`,
          );
        }
      }, delay);
    } else {
    ...
    if (isFirstInstance) {
      const verificationInterval = setInterval(() => {
        ...
        if (GLOBAL_STATE.isConnected) {
          ...
          if (!canSendMessages()) {
            console.warn(
              "WebSocket: Connection verification failed - socket not ready despite connected state",
            );
            updateConnectionState(false);
          ...
          console.log(
            "WebSocket: No active connection detected during verification, attempting to connect",
          );
          initializeSocket();
        }
      ...
      return () => {
        ...
        GLOBAL_STATE.instanceCount--;
        console.log(
          `WebSocket hook instance destroyed (${instanceId}). Remaining instances: ${GLOBAL_STATE.instanceCount}`,
        );

        // Only close the socket when no components are using it
        if (GLOBAL_STATE.instanceCount <= 0) {
          console.log(
            "Closing WebSocket connection as no components are using it",
          );
          if (GLOBAL_STATE.socket) {
            GLOBAL_STATE.socket.close();
            GLOBAL_STATE.socket = null;
  ...
  const addEventListener = useCallback(
    <T>(event: WebSocketEvent, handler: (data: T) => void): (() => void) => {
