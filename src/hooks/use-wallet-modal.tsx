...
export interface SIWS {
  ...
  verify: (args: { payload: any; signature: any }) => Promise<{ success: boolean; error?: { type: string } }>;
}
...
export function useWalletAuthentication() {
  ...
  useEffect(() => {
    const checkAuthStatus = async () => {
      ...
      try {
        console.log("Checking auth status for wallet:", publicKey.toString().slice(0, 10) + "...");
        ...
        if (response.ok) {
          ...
          if (isAuthenticated !== data.authenticated) {
            ...
            if (!data.authenticated && authToken) {
              console.log("Server reports not authenticated, clearing local token");
      ...
      if (authData.token) {
        ...
        try {
          ...
          if (authCheckResponse.ok) {
            const statusData = await authCheckResponse.json() as { authenticated: boolean };
