...
export default function useAuthentication() {
  ...
  const instanceIdRef = useRef<string>(
    `auth-${Math.random().toString(36).substring(2, 11)}`,
  );

  // Set auth token and store it in localStorage in enhanced format
  const setAuthTokenWithStorage = useCallback(
    (token: string | null) => {
      if (token) {
        setAuthToken(token);
        ...
        if (storedWalletAddress || publicKey?.toString()) {
          const walletAddress =
            storedWalletAddress || publicKey?.toString() || "";
          try {
  ...
  const getWalletAddressFromToken = useCallback(
  ...
  const checkAuthStatus = useCallback(async () => {
    ...
    const now = Date.now();
    if (
      now - GLOBAL_AUTH_STATE.lastCheckTime <
      GLOBAL_AUTH_STATE.CHECK_COOLDOWN
    ) {
      console.log(
        `Skipping auth check - last check was ${(now - GLOBAL_AUTH_STATE.lastCheckTime) / 1000}s ago`,
      );
      return;
    }
  ...
  useEffect(() => {
    ...
    console.log(
      `Auth component mounted: ${instanceId}. Total: ${GLOBAL_AUTH_STATE.mountedComponents.size}`,
    );
    ...
    return () => {
      ...
      console.log(
        `Auth component unmounted: ${instanceId}. Remaining: ${GLOBAL_AUTH_STATE.mountedComponents.size}`,
      );
  ...
  useEffect(() => {
    ...
    if (GLOBAL_AUTH_STATE.checkInProgress) {
      console.log(
        `Auth check already in progress, skipping from ${instanceId}`,
      );
      return;
    }
    ...
    if (authCheckSentViaWebSocket.current) {
      console.log(
        `Auth check already sent via WebSocket, skipping HTTP check from ${instanceId}`,
      );
      return;
    }
    ...
    if (
      ...
      if (GLOBAL_AUTH_STATE.initialConnectionPhase) {
        console.log(
          `In initial connection phase, waiting for WebSocket before auth check. Instance: ${instanceId}`,
        );
      } else {
        console.log(
          `In extended initial phase (tokens received via WS), skipping HTTP auth. Instance: ${instanceId}`,
        );
      }
      ...
      if (!GLOBAL_AUTH_STATE.initialPhaseTimeout) {
        GLOBAL_AUTH_STATE.initialPhaseTimeout = setTimeout(() => {
          ...
          if (!GLOBAL_AUTH_STATE.tokensReceivedViaWebSocket) {
            ...
            console.log(
              "Tokens received via WebSocket, maintaining extended phase",
            );
            GLOBAL_AUTH_STATE.initialConnectionPhase = false;
            GLOBAL_AUTH_STATE.extendedInitialPhase = true;
            ...
            setTimeout(() => {
              console.log(
                "Extended initial phase ended (after tokens received)",
              );
              GLOBAL_AUTH_STATE.extendedInitialPhase = false;
      ...
      if (GLOBAL_AUTH_STATE.tokensReceivedViaWebSocket) {
        console.log(
          "Tokens received via WebSocket, skipping HTTP auth completely",
        );
        setIsAuthenticating(false);
        return;
      }
      ...
      const timeSinceLastWSActivity =
        Date.now() - GLOBAL_AUTH_STATE.lastWebSocketActivity;
      if (
        GLOBAL_AUTH_STATE.authRequestedViaWebSocket &&
        timeSinceLastWSActivity < 5000
      ) {
        console.log(
          `Auth already requested via WebSocket ${timeSinceLastWSActivity}ms ago, skipping HTTP check`,
        );
        setIsAuthenticating(false);
        return;
      }
    ...
    const now = Date.now();
    if (
      GLOBAL_AUTH_STATE.checkStatusCalled &&
      now - GLOBAL_AUTH_STATE.lastCheckTime < GLOBAL_AUTH_STATE.CHECK_COOLDOWN
    ) {
      console.log(
        `Auth check already performed within cooldown period. Instance: ${instanceId}`,
      );
      setIsAuthenticating(false);
      return;
    }
    ...
    if (wsConnected) {
      console.log(
        `Using WebSocket for auth status, no polling needed. Instance: ${instanceId}`,
      );
      return;
    }
    ...
    if (
      GLOBAL_AUTH_STATE.mountedComponents.size > 0 &&
      instanceId !== Array.from(GLOBAL_AUTH_STATE.mountedComponents)[0]
    ) {
      console.log(
        `Polling already set up by another instance, not duplicating. Instance: ${instanceId}`,
      );
      return;
    }
    ...
    console.log(
      `WebSocket not connected, setting up polling for auth status. Instance: ${instanceId}`,
    );
    const interval = setInterval(
      () => {
        ...
        if (
          !wsConnected &&
          !GLOBAL_AUTH_STATE.checkInProgress &&
          !GLOBAL_AUTH_STATE.initialConnectionPhase &&
          !GLOBAL_AUTH_STATE.extendedInitialPhase
        ) {
      ...
      5 * 60 * 1000,
    ); // 5 minute polling interval as fallback
