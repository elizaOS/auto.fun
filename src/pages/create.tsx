...
const waitForTokenCreation = async (mint: string, timeout = 80_000) => {
  return new Promise<void>((resolve, reject) => {
    ...
    const newTokenListener = (token: unknown) => {
      console.log("WebSocket newToken received:", token);
      try {
        console.log(
          "Attempting to parse with HomepageTokenSchema:",
          JSON.stringify(token)
        );
        const { mint: newMint } = HomepageTokenSchema.parse(token);
        console.log("Successfully parsed token, mint:", newMint);
        if (newMint === mint) {
        ...
        // Try to access mint directly without schema validation as a fallback
        try {
          const { mint: newMint } = token as { mint: string };
          console.log("Fallback: extracted mint directly:", newMint);
          if (newMint === mint) {
            clearTimeout(timerId);
            socket.off("newToken", newTokenListener);
            resolve();
