    ...
    <div className="p-4 bg-autofun-background-input ">
              ...
              <tr key={user.id} className="border-b border-autofun-background-primary">
                ...
                <td className="p-2">
                  <span className={`px-2 py-1 rounded-full text-xs ${
                    ...
                    {user.name?.startsWith('[SUSPENDED]') ? 'suspended' : 'active'}
                  </span>
                </td>
                <td className="p-2">
                  <Link 
      ...
      <div className="flex justify-between items-center pt-4">
        <div className="text-sm text-autofun-text-secondary">
          ...
          {usersPagination.totalItems || 0} users
        </div>
...
function AdminUserDetails({ address }: { address: string }) {
  ...
  const userQuery = useQuery<User>({
    ...
    queryFn: async () => {
      const response = await fetcher(`/api/admin/users/${address}`, "GET") as { user: User };
    ...
    return (
      <div className="p-4 bg-autofun-background-input ">
        ...
        <div className="p-4 bg-red-900/20 text-red-300 ">
          <p>Error loading user data. The user may not exist or there was an error fetching the data.</p>
        </div>
      </div>
    );
  }
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async (suspended: boolean) => {
      return await fetcher(`/api/admin/users/${address}/suspended`, "POST", { suspended });
    },
    onSuccess: () => {
