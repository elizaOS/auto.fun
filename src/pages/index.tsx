...
export default function Page() {
  ...
  const { addEventListener, connected: wsConnected, sendMessage, requestTokenList, isTokenRequestActive } = useGlobalWebSocket();
  ...
  useEffect(() => {
    if (wsConnected) {
      console.log(`Requesting tokens via WebSocket for page=${page}, sortBy=${sortBy}, sortOrder=${sortOrder}`);
  ...
  useEffect(() => {
    ...
    const unsubscribeTokensList = addEventListener<TokenResponse>("tokensList", (data) => {
      if (data && data.tokens) {
        console.log("Received tokens list via WebSocket:", data.tokens.length, "tokens");
        ...
        queryClient.setQueryData(["tokens", data.page, sortBy, sortOrder], tokenData);
      }
    });
    
    // Listen for new tokens
    const removeNewTokenListener = addEventListener<IToken>("newToken", (token) => {
      ...
      if (page === 1 && sortBy === "createdAt") {
        setWsTokensData((prev) => {
          ...
          queryClient.setQueryData(["tokens", page, sortBy, sortOrder], updatedData);
    ...
    const removeSwapListener = addEventListener<SwapEvent>("newSwap", (swap) => {
      ...
      if (sortBy === "featured" || sortBy === "marketCapUSD") {
        ...
        console.log("Swap may affect rankings, requesting updated token list");
        requestTokenList(page, 12, sortBy, sortOrder);
      }
    });
  ...
  }, [addEventListener, queryClient, wsConnected, page, sortBy, sortOrder, requestTokenList]);
  ...
  const tokensData = wsConnected && wsTokensData 
    ? wsTokensData 
