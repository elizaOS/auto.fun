    ...
    if (visible) {
      try {
        ...
        if (storedWalletName) {
          ...
          setPreviouslySelectedWallet(parsedWalletName);
  ...
  const mutation = useMutation({
    ...
    mutationFn: async ({ wallet }: { wallet: Wallet }) => {
      try {
        ...
        console.log(`Connecting to ${isPhantom ? 'Phantom' : wallet.adapter.name} wallet...`);
        ...
        if (!directConnectionSuccessful && !connectedWallet) {
          ...
          try {
            ...
            if (!isPhantom || !window.solana || !window.solana.publicKey) {
              ...
              console.log("Using successful direct connection despite adapter error");
            }
          }
        }
        ...
        let finalPublicKey = isPhantom && window.solana && window.solana.publicKey
        ...
        const publicKeyStr = typeof finalPublicKey === 'string' 
          ? finalPublicKey 
