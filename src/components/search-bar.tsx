...
export default function SearchBar() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = addEventListener<{ tokens: IToken[] }>(
      "searchResults" as WebSocketEvent,
      (data) => {
        if (data && data.tokens) {
          console.log(
            "Received search results via WebSocket:",
            data.tokens.length,
            "tokens",
          );
          ...
          if (search) {
            queryClient.setQueryData(["search-tokens", search], {
              tokens: data.tokens,
            });
          }
