...
export function SolPriceProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (connected) {
      console.log("SolPriceProvider: WebSocket connected, requesting SOL price update");
      requestSolPrice();
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (initialLoading) {
        console.log("SolPriceProvider: Initialization timeout reached, using query data");
        setInitialLoading(false);
      }
