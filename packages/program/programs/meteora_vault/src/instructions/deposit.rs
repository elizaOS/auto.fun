...
pub struct Deposit<'info> {
    ...
    // Position token mint
    pub position_nft: Box<InterfaceAccount<'info, Mint>>,

    // Account that holds the NFT to deposit
    #[account(
        mut,
        associated_token::mint = position_nft,
    ...
    pub nft_token_faucet: Box<InterfaceAccount<'info, TokenAccount>>,
...
pub fn process_deposit(ctx: Context<Deposit>, claimer_address: Pubkey) -> Result<()> {
    ...
    if position.claimer.eq(&Pubkey::default()) {
        ...
        require!(position.amount == 0, VaultError::OnlyOneNftAllowed);
    ...
    msg!("Transferring NFT to vault pda");
    ...
    anchor_spl::token_interface::transfer_checked(
        CpiContext::new(
            ...
            anchor_spl::token_interface::TransferChecked {
                ...
                mint: ctx.accounts.position_nft.to_account_info(),
            }
        ),
